...
import { effectiveBudget } from "../src/observability/budget";
import { unknownCommandHint } from "../src/lifecycle/commands";
import { loadDotEnv, selectProjectKeys } from "../src/mcp/server";
...
test("jawnie pusta lista projektów jest fail-closed, brak zmiennej zachowuje configured", () => {
  assert.deepEqual(selectProjectKeys({}, ["alpha", "beta"]), ["alpha", "beta"]);
  assert.deepEqual(
    selectProjectKeys({ LINEAR_PROJECTS: "" }, ["alpha", "beta"]),
    []
  );
  assert.deepEqual(
    selectProjectKeys({ LINEAR_PROJECT: "" }, ["alpha", "beta"]),
    []
  );
  assert.deepEqual(
    selectProjectKeys(
      { LINEAR_PROJECTS: " alpha , , beta " },
      ["alpha", "beta", "gamma"]
    ),
    ["alpha", "beta"]
  );
  assert.deepEqual(
    selectProjectKeys(
      { LINEAR_PROJECTS: "", LINEAR_PROJECT: "alpha" },
      ["alpha", "beta"]
    ),
    []
  );
...
test("openGate odwzorowuje approval, advisory-fix, blocked i done", () => {
  ...
  assert.deepEqual(openGate(run({
    ...
    gate: "merge",
    waitingOn: "human",
    humanCommands: ["/replan <powód>"],
  });
  assert.ok(openGate(run({
    stage: "merge",
    status: "waiting_human",
  })).humanCommands.includes("/score 1-5"));
  assert.deepEqual(openGate(run({
    stage: "merge",
    status: "waiting_human",
  ...
  const reachableScoredHint = unknownCommandHint({
    firstToken: "/scroe",
    stage: "merge",
    status: "waiting_human",
    score: 4,
...
test("effectiveBudget waliduje env i override projektu oraz zasila projekcje MCP", async () => {
  const previousMinutes = process.env.FACTORY_BUDGET_MAX_MIN;
  const previousUsd = process.env.FACTORY_BUDGET_MAX_USD;
  const noBudgetProject = { ...project };
  delete noBudgetProject.budget;
  try {
    for (const invalid of ["abc", "0", "-1"]) {
      process.env.FACTORY_BUDGET_MAX_MIN = invalid;
      process.env.FACTORY_BUDGET_MAX_USD = invalid;
      assert.deepEqual(effectiveBudget(noBudgetProject), { maxMinutes: 45, maxUsd: 3 });
    }

    process.env.FACTORY_BUDGET_MAX_MIN = "60";
    process.env.FACTORY_BUDGET_MAX_USD = "5.5";
    assert.deepEqual(effectiveBudget(noBudgetProject), { maxMinutes: 60, maxUsd: 5.5 });

    process.env.FACTORY_BUDGET_MAX_MIN = "abc";
    process.env.FACTORY_BUDGET_MAX_USD = "abc";
    const configuredProject = {
      ...project,
      budget: { maxMinutes: 90, maxUsd: 12 },
    };
    const limits = effectiveBudget(configuredProject);
    assert.deepEqual(limits, { maxMinutes: 90, maxUsd: 12 });

    const store = new FakeStore();
    store.runs.set("BAR-200", run());
    const tools = createFactoryTools(deps({
      store,
      projects: { harness: configuredProject },
    }));
    const projects = await tools.factoryProjects();
    const status = await tools.ticketStatus({ ticket: "BAR-200" });
    assert.deepEqual(projects.projects[0]?.budget, limits);
    assert.equal(status.found, true);
    if (status.found) {
      assert.equal(status.budgetMaxMinutes, limits.maxMinutes);
      assert.equal(status.budgetMaxUsd, limits.maxUsd);
    }
  } finally {
    if (previousMinutes === undefined) delete process.env.FACTORY_BUDGET_MAX_MIN;
    else process.env.FACTORY_BUDGET_MAX_MIN = previousMinutes;
