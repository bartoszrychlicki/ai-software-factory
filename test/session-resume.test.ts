...
import type { EngineRunInput } from "../src/adapters/engines/types";
...
test("claude-code zachowuje cold argv, dodaje --resume i czyta session_id z init/result", async () => {
  ...
  try {
    ...
    const { claudeCode } = await import("../src/adapters/engines/claude-code");
...
test("prompt rundy resumed zawiera tylko ostatnią odpowiedź, a brak sesji zachowuje pełny fallback", async () => {
  const { buildPlanEnginePrompt } = await import("../src/legacy/ticket-pipeline");
  const ticket = {
    id: "BAR-136",
...
test("planStep przenosi sessionId przez suspend/resume i zapisuje metryki cold/resumed", async () => {
  ...
  try {
    ...
    const [{ engines }, { ticketPipeline }, { Mastra }, { LibSQLStore }, { applyWorkflowPersistencePatch }] = await Promise.all([
      import("../src/adapters/engines"),
      import("../src/legacy/ticket-pipeline"),
      import("@mastra/core/mastra"),
      import("@mastra/libsql"),
      import("../src/mastra/workflow-persistence-patch"),
    ]);
...
test("report pokazuje rozbicie planu na resumed=false i resumed=true", () => {
  ...
  try {
    ...
    const output = execFileSync(process.execPath, ["--import", "tsx", join(factoryDir, "src/observability/report.ts")], {
      cwd: factoryDir,
      encoding: "utf8",
...
test("fake-CLI benchmark zapisuje kontrolowany artefakt A/B z proxy jakości", () => {
  ...
  try {
    ...
    execFileSync(
      process.execPath,
      ["--import", "tsx", join(factoryDir, "src/legacy/bench-plan-resume.ts"), "BAR-136", scenarioPath],
      {
        cwd: factoryDir,
        encoding: "utf8",
