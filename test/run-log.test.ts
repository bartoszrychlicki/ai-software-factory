...
import {
  ...
  defaultRunLogIo,
  writeRunLog,
} from "../src/observability/run-log";
...
async function seedCompletedRun(harness: Harness, ticketId: string): Promise<LifecycleRun> {
  ...
  store.startAttempt(ticketId, "plan", 1, "job-plan-g1", { generation: 1 });
  ...
  store.startAttempt(ticketId, "plan", 2, "job-plan-g2", { generation: 2 });
  ...
  store.startAttempt(ticketId, "build", 1, "job-build-g2", { generation: 2 });
  ...
  store.startAttempt(ticketId, "review", 1, "job-review-g2", { generation: 2 });
...
test("atomowość: błąd writeFile zachowuje stary przebieg i nie zostawia tmp", async () => {
  await withHarness("factory-run-log-atomic-write-", async ({ runsRoot, store }) => {
    const ticketId = "BAR-LOG-ATOMIC-WRITE";
    const dir = join(runsRoot, ticketId);
    const path = join(dir, "przebieg.md");
    await mkdir(dir, { recursive: true });
    await writeFile(path, "stara treść\n");
    store.createRun(ticketId, "harness", manifest);
    const errors: unknown[][] = [];
    const originalError = console.error;
    console.error = (...args: unknown[]) => { errors.push(args); };
    try {
      assert.doesNotThrow(() => writeRunLog(store, store.getRun(ticketId)!, {
        ...defaultRunLogIo,
    ...
    assert.equal(await readFile(path, "utf8"), "stara treść\n");
    assert.deepEqual((await readdir(dir)).filter((name) => name.startsWith("przebieg.md.tmp-")), []);
    assert.ok(errors.some((args) => String(args[0]).includes(`Przebieg ${ticketId} nie zapisany`)));
  });
});

test("atomowość: błąd rename zachowuje stary przebieg i sprząta tmp", async () => {
  await withHarness("factory-run-log-atomic-rename-", async ({ runsRoot, store }) => {
    const ticketId = "BAR-LOG-ATOMIC-RENAME";
    const dir = join(runsRoot, ticketId);
    const path = join(dir, "przebieg.md");
    await mkdir(dir, { recursive: true });
    await writeFile(path, "stara treść\n");
...
test("atomowość: sukces podmienia przebieg i nie zostawia tmp", async () => {
  await withHarness("factory-run-log-atomic-success-", async ({ runsRoot, store }) => {
    const ticketId = "BAR-LOG-ATOMIC-SUCCESS";
    const dir = join(runsRoot, ticketId);
    const path = join(dir, "przebieg.md");
    await mkdir(dir, { recursive: true });
    await writeFile(path, "stara treść\n");
    ...
    assert.match(await readFile(path, "utf8"), /^# BAR-LOG-ATOMIC-SUCCESS — przebieg ticketu/m);
    assert.deepEqual((await readdir(dir)).filter((name) => name.startsWith("przebieg.md.tmp-")), []);
  });
});
...
test("dwie generacje linkują wyłącznie własne artefakty lokalnych testów", async () => {
  await withHarness("factory-run-log-local-test-generations-", async ({ runsRoot, store, deps }) => {
    const ticketId = "BAR-LOG-LOCAL-TEST";
    const ticketDir = join(runsRoot, ticketId);
    await mkdir(ticketDir, { recursive: true });
    await Promise.all([
      writeFile(join(ticketDir, "test-result-g1-a1.json"), "{}"),
      writeFile(join(ticketDir, "test-result-g1-a1.json.input.json"), "{}"),
      writeFile(join(ticketDir, "test-runner-g1-a1.log"), "g1"),
      writeFile(join(ticketDir, "test-result-g2-a2.json"), "{}"),
      writeFile(join(ticketDir, "test-result-g2-a2.json.input.json"), "{}"),
      writeFile(join(ticketDir, "test-runner-g2-a2.log"), "g2"),
    ]);

    store.createRun(ticketId, "harness", manifest);
    store.startAttempt(ticketId, "test", 1, "local-test:g1:1", { generation: 1 });
    store.finishAttempt(ticketId, "test", 1, {
      status: "success",
      outcome: "pass",
      signature: "test-runner · g1",
    });
    applyTransition(deps, ticketId, {
      stage: "plan",
      status: "running",
      actor: "human",
      reason: "/replan c-local",
      incrementGeneration: true,
    });
    store.startAttempt(ticketId, "test", 2, "local-test:g2:2", { generation: 2 });
    store.finishAttempt(ticketId, "test", 2, {
      status: "success",
      outcome: "pass",
      signature: "test-runner · g2",
    });
    applyTransition(deps, ticketId, {
      stage: "smoke",
      status: "done",
    ...
    const g1Row = log.split("\n").find((line) => line.includes("test-runner · g1"))!;
    const g2Row = log.split("\n").filter((line) => line.includes("test-runner · g2")).at(-1)!;
    assert.match(g1Row, /test-result-g1-a1\.json/);
    assert.match(g1Row, /test-result-g1-a1\.json\.input\.json/);
    assert.match(g1Row, /test-runner-g1-a1\.log/);
    assert.doesNotMatch(g1Row, /-g2-/);
    assert.match(g2Row, /test-result-g2-a2\.json/);
    assert.match(g2Row, /test-result-g2-a2\.json\.input\.json/);
    assert.match(g2Row, /test-runner-g2-a2\.log/);
    assert.doesNotMatch(g2Row, /-g1-/);
  });
});

test("legacy log runnera jest linkowany tylko przy dowiedzionej jednej generacji", async () => {
  await withHarness("factory-run-log-local-test-legacy-", async ({ runsRoot, store, deps }) => {
    const ticketId = "BAR-LOG-LEGACY-ONE";
    const ticketDir = join(runsRoot, ticketId);
    await mkdir(ticketDir, { recursive: true });
    await writeFile(join(ticketDir, "test-runner-a1.log"), "legacy");
    store.createRun(ticketId, "harness", manifest);
    store.startAttempt(ticketId, "test", 1, "local-test:legacy:1", { generation: 1 });
    store.finishAttempt(ticketId, "test", 1, {
      status: "success",
      signature: "test-runner · legacy-one",
    });
    applyTransition(deps, ticketId, {
      stage: "smoke",
      status: "done",
    ...
    assert.match(
      await readFile(join(ticketDir, "przebieg.md"), "utf8"),
      /\.\/test-runner-a1\.log/
    );

    const ambiguousId = "BAR-LOG-LEGACY-MANY";
    const ambiguousDir = join(runsRoot, ambiguousId);
    await mkdir(ambiguousDir, { recursive: true });
    await writeFile(join(ambiguousDir, "test-runner-a1.log"), "legacy");
    store.createRun(ambiguousId, "harness", manifest);
    store.startAttempt(ambiguousId, "test", 1, "local-test:legacy-many:1", { generation: 1 });
    store.finishAttempt(ambiguousId, "test", 1, {
      status: "success",
      signature: "test-runner · legacy-many",
    });
    applyTransition(deps, ambiguousId, {
      stage: "plan",
      status: "running",
      actor: "human",
      reason: "/replan c-legacy",
      incrementGeneration: true,
    });
    const ambiguous = await readFile(join(ambiguousDir, "przebieg.md"), "utf8");
    assert.doesNotMatch(ambiguous, /\.\/test-runner-a1\.log/);
    assert.match(ambiguous, /pominięto legacy log runnera — niejednoznaczna generacja/);
  });
});

test("lokalna próba bez generacji pomija artefakty z jawną notą", async () => {
  await withHarness("factory-run-log-local-test-unknown-generation-", async ({ runsRoot, store }) => {
    const ticketId = "BAR-LOG-UNKNOWN-GENERATION";
    const ticketDir = join(runsRoot, ticketId);
    await mkdir(ticketDir, { recursive: true });
    await writeFile(join(ticketDir, "test-result-g1-a1.json"), "{}");
    store.createRun(ticketId, "harness", manifest);
    store.startAttempt(ticketId, "test", 1, "local-test:legacy:1");
    store.finishAttempt(ticketId, "test", 1, {
      status: "success",
      signature: "test-runner · unknown-generation",
    });
    const log = buildRunLog(store, store.getRun(ticketId)!);
    assert.match(log, /brak artefaktów \(nieznana generacja\)/);
    assert.doesNotMatch(log, /\.\/test-result-g1-a1\.json/);
  });
});
