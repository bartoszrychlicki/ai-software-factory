...
test("czytelnik widzi run i nie modyfikuje pliku bazy", () => {
  ...
  try {
    ...
    writer.createRun("SECRET-RO-1", "secret", manifest);
    writer.startAttempt("BAR-RO-1", "build", 1, "job-visible");
    writer.finishAttempt("BAR-RO-1", "build", 1, { status: "success", costUsd: 1.25 });
    writer.startAttempt("SECRET-RO-1", "build", 1, "job-secret");
    writer.finishAttempt("SECRET-RO-1", "build", 1, { status: "success", costUsd: 9 });
    writer.close();
    const before = statSync(path).mtimeMs;

    const reader = new LifecycleStore(path, { readOnly: true });
    assert.equal(reader.getRun("BAR-RO-1")?.ticketId, "BAR-RO-1");
    assert.equal(reader.usageSinceForProjects(new Date(0).toISOString(), ["harness"]), 1.25);
    assert.equal(reader.usageSinceForProjects(new Date(0).toISOString(), []), 0);
    reader.close();
