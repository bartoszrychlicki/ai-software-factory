...
import { reduceLifecycle, type CoordinatorEvent, type NextAttempts } from "../src/lifecycle/coordinator";
import {
  executeFactoryJobInput,
  type FactoryJobOutput,
  type FactoryJobRuntime,
} from "../src/jobs/factory-job";
import { parseCritiqueVerdict, parseTriageVerdict } from "../src/lifecycle/verdicts";
import { parseCommand, parseScorePayload } from "../src/lifecycle/commands";
import { dispatchOutbox, type PollerDependencies } from "../src/app/poller";
import type { EngineAdapter } from "../src/adapters/engines/types";
...
test("incydent BAR-180: etykieta przed JSON-em w bloku factory nie gubi werdyktu", async () => {
  const { verdictInstruction } = await import("../src/lifecycle/verdicts");
