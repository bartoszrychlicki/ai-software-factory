...
import type { EngineAdapter, EngineRunResult } from "../src/adapters/engines/types";
import {
  classifyEngineFailure,
  classifyEngineRunFailure,
} from "../src/lifecycle/failure-classes";
import {
  decideFallback,
  executeFactoryJobInput,
  type FactoryJobRuntime,
} from "../src/jobs/factory-job";
import type { Route } from "../src/config/routing";
...
function runAdapterInChild(
  ...
  const moduleUrl = pathToFileURL(
    join(factoryDir, "src", "adapters", "engines", `${moduleName}.ts`)
  ).href;
  const source = [
