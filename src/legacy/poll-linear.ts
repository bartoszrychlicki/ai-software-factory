 ...
 *
 * Uruchomienie historyczne: npx tsx src/legacy/poll-linear.ts [--once]
...
} from "../adapters/linear/comment-context";
import { readRetryEvidence } from "../adapters/linear/retry-context";
import {
...
import { breakerOpen, recordRunOutcome, checkHourlySpend } from "../observability/breaker";
import { notify } from "../observability/notify";
import { runProdChecks } from "../execution/prod-smoke";
import {
  classifyOpsVerification,
  isOpsWorkflowResult,
  opsTrackerStatus,
  runProdChecksWithRetry,
} from "../execution/ops-checklist";
import * as registry from "./run-registry";
import { LINEAR_STATE_MAP as MAP, decisionOfState, validateStateMap } from "../lifecycle/state-map";
import { parseCommand, hintFor, isDecisionCommand } from "../lifecycle/commands";
import { parsePlanVerdict, resolveDomain } from "../lifecycle/verdicts";
import { allQualityCommands, QualityGateError, runQualityCommands } from "../execution/quality";
import {
  parseSignatureMeta,
  POLLER_SIGNATURE,
...
import { acknowledgeOutboxFromRun, dispatchPendingOutbox, outboxId } from "./mastra-outbox";
import { MissingRunDetector, processRestartCommands } from "./run-recovery";
import { currentTrackedPrUrl } from "../adapters/linear/merge-tracking";
...
if (!API_KEY) {
  console.error("Brak LINEAR_API_KEY (env lub .env w katalogu repozytorium)");
  process.exit(1);
}
...
/** Minimalny loader .env — bez nowej zależności. Szuka w root repo. */
function loadDotEnv() {
  try {
