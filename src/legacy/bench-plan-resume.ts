 ...
 *   npx tsx src/legacy/bench-plan-resume.ts BAR-136 ./scenario.json
 *
 * scenario.json:
 * {
...
import { z } from "zod";
import { claudeCode } from "../adapters/engines/claude-code";
import type { EngineRunResult } from "../adapters/engines/types";
import { buildPlanEnginePrompt } from "../legacy/ticket-pipeline";
import { findUpFile } from "../config/projects";
import { parsePlanVerdict } from "../lifecycle/verdicts";
...
async function main(): Promise<void> {
  ...
  if (!ticket || !scenarioArg) {
    throw new Error("Użycie: npx tsx src/legacy/bench-plan-resume.ts <ticket> <scenario.json>");
  }
