...
/** Fail-closed: jawnie pusta zmienna zawęża do zera, nigdy nie rozszerza do wszystkich projektów. */
export function selectProjectKeys(
  env: Record<string, string | undefined>,
  configuredKeys: string[]
): string[] {
  const present = (name: string) =>
    Object.hasOwn(env, name) && typeof env[name] === "string";
  const raw = present("LINEAR_PROJECTS")
    ? env.LINEAR_PROJECTS
    : present("LINEAR_PROJECT")
      ? env.LINEAR_PROJECT
      : undefined;
  if (raw === undefined) return configuredKeys;
...
async function loadProjects(): Promise<Record<string, ProjectConfig>> {
  ...
  const selectedKeys = selectProjectKeys(process.env, configuredKeys);
