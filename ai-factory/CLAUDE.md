...
9. **Dwie rundy QA** (decyzja Bartosza 2026-07-21, PRIORYTET — wniosek z BAR-101/102: verify PASS + LGTM, a funkcja martwa na produkcji, bo domena serwuje stary hosting; definicja done fabryki kończyła się na granicy repo). Runda 2 WAŻNIEJSZA: prod smoke po merge'u — merge-watcher po Done czeka na deploy i odpala per-projektowy check (qa.prodCheck: curl/e2e:prod/screenshot prod URL), wynik komentarzem; FAIL = alarm + reopen zamiast cichego Done. Runda 1: e2e projektu w verify (br-budget ma infrę playwright; zbadać e2e:local bez sekretów prod). 
...
11. **Tickety klasy ops/infra** (wniosek z BAR-102): rozpoznawać na bramce planu i kierować w tryb „checklist dla człowieka + weryfikacja fabryki" zamiast pętli build→verify, która ich z definicji nie spełni (kryteria poza repo: DNS, panele, sekrety).
12. Webhooki Lineara zamiast pollingu (dziś opóźnienia do 60 s na podjęcie / 20 s na aprobatę — wystarcza; wraca przy skali).
