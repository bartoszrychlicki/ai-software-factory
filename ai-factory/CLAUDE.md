...
- **Studio pokazuje stale snapshot po resume — bug w @mastra/core ≤1.52-alpha**: persistStepUpdate ma guard odrzucający zapisy `running`, gdy w in-process mapie lastPersistedStatus wisi `suspended`; mapa czyści się dopiero na KOŃCU runa, więc po resume żaden krok się nie persystuje aż do finału. Miał chronić przed wyścigiem przy bootowaniu — blokuje legalne aktualizacje. Zbadane 2026-07-21 (kod chunk-77JDY5O7.js), alpha 1.52 bez zmian. Decyzja: NIE patchujemy vendora; źródłem prawdy o postępie są komentarze Linear + logi pollera + artefakty, docelowo statusy Linear (backlog #10). Issue upstream do zgłoszenia przez Bartosza.
