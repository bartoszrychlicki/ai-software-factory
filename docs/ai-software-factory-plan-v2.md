...
4. **Routing = konfiguracja, nie kod** — `routing.yaml`: etap × domena → silnik, np. `build.frontend: kimi-code`. Override ręczny labelem na tickecie (`engine:kimi`). Router **nie jest LLM-em** w MVP — to reguły.
